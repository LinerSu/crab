...
class object_info {
  // Simple reduced product of (SmallRange x Boolean x Boolean x Boolean)
  using product_t =
      basic_domain_product2<small_range, basic_domain_product2<boolean_value,
...
public:
  ...
  object_info(small_range count, boolean_value is_copied, boolean_value cache_used, boolean_value cache_dirty) {
    refcount_val() = count;
    iscopied_val() = is_copied;
  ...
  // Object is copied from another. (Used by interprecedural analysis)
  boolean_value &iscopied_val() { return m_product.second().first(); }
  // Whether the cache domain is used.
  boolean_value &cacheused_val() { return m_product.second().second().first(); }
  // Whether the cache domain has been updated.
  boolean_value &cachedirty_val() { return m_product.second().second().second(); }
  const small_range &refcount_val() const { return m_product.first(); }
  const boolean_value &iscopied_val() const { return m_product.second().first(); }
  const boolean_value &cacheused_val() const { return m_product.second().second().first(); }
  const boolean_value &cachedirty_val() const { return m_product.second().second().second(); }
